...
import { MarketAddress, MarketAddressABI, MarketAddressAbi } from './constants';
...
export const NFTProvider = ({ children }) => {
  ...
  const checkIfWalletIsConnected = async () => {
    if (!window.ethereum) return alert('Por Favor conecta Metamask');
  ...
  const uploadToIPFS = async (file) => {
    try {
      ...
      console.log('Error cargando archivo IPFS');
    }
  };
  ...
  const createNFT = async (formInput, fileUrl, router) => {
    ...
    try {
      ...
      console.log(error);
      console.log('Error cargando archivo a IPFS');
    }
  };
